...
class WC_Shipping_Fedex extends WC_Shipping_Method {
    ...
    private function init() {
		...
		$this->freight_enabled = isset( $this->settings['freight_enabled'] ) && $this->settings['freight_enabled'] == 'yes' ? true : false;
    ...
    public function init_form_fields() {
    	...
    	$this->form_fields  = array(
		    ...
		    'freight_class'           => array(
				'title'           => __( 'Fedex Freight Class', 'wc_fedex' ),
				'type'            => 'select',
				'default'         => 'CLASS_050',
				'options'         => array(
					'CLASS_050'   => 'CLASS_050',
    ...
    public function calculate_shipping( $package ) {
    	...
    	if ( $package_requests ) {
    		...
    		$packages_to_quote_count = sizeof( $package_requests );
			...
			try {
				...
				if ( $this->freight_enabled ) {
					$package_requests = $this->get_freight_requests( $package_requests, $package );
     ...
     *
     * @access private
     * @param mixed $package_requests
     * @return void
     */
    private function get_freight_requests( $package_requests, $package ) {
	    global $woocommerce;

    	$freight_requests = array();

    	$freight_number           = isset( $this->settings['freight_number'] ) ? $this->settings['freight_number'] : $this->account_number;

    	$freight_billing_street   = isset( $this->settings['freight_billing_street'] ) ? $this->settings['freight_billing_street'] : '';
    	$freight_billing_street_2   = isset( $this->settings['freight_billing_street_2'] ) ? $this->settings['freight_billing_street_2'] : '';
	    $freight_billing_city     = isset( $this->settings['freight_billing_city'] ) ? $this->settings['freight_billing_city'] : '';
	    $freight_billing_state    = isset( $this->settings['freight_billing_state'] ) ? $this->settings['freight_billing_state'] : '';
	    $freight_billing_postcode = isset( $this->settings['freight_billing_postcode'] ) ? $this->settings['freight_billing_postcode'] : '';
	    $freight_billing_country  = isset( $this->settings['freight_billing_country'] ) ? $this->settings['freight_billing_country'] : '';

	    $freight_shipper_street   = isset( $this->settings['freight_shipper_street'] ) ? $this->settings['freight_shipper_street'] : '';
	    $freight_shipper_street_2   = isset( $this->settings['freight_shipper_street_2'] ) ? $this->settings['freight_shipper_street_2'] : '';
	    $freight_shipper_city     = isset( $this->settings['freight_shipper_city'] ) ? $this->settings['freight_shipper_city'] : '';
	    $freight_shipper_state    = isset( $this->settings['freight_shipper_state'] ) ? $this->settings['freight_shipper_state'] : '';
	    $freight_shipper_postcode = isset( $this->settings['freight_shipper_postcode'] ) ? $this->settings['freight_shipper_postcode'] : '';
	    $freight_shipper_country  = isset( $this->settings['freight_shipper_country'] ) ? $this->settings['freight_shipper_country'] : '';

	    $freight_shipper_residential = isset( $this->settings['freight_shipper_residential'] ) && $this->settings['freight_shipper_residential'] == 'yes' ? true : false;

	    $freight_class            = isset( $this->settings['freight_class'] ) ? $this->settings['freight_class'] : '';

    	foreach ( $package_requests as $freight_request ) {

    		$freight_request['RequestedShipment']['Shipper'] = array(
			    'Address'               => array(
			    	'StreetLines'         => array( strtoupper( $freight_shipper_street ), strtoupper( $freight_shipper_street_2 ) ),
					'City'                => strtoupper( $freight_shipper_city ),
					'StateOrProvinceCode' => strtoupper( $freight_shipper_state ),
					'PostalCode'          => strtoupper( $freight_shipper_postcode ),
					'CountryCode'         => strtoupper( $freight_shipper_country ),
					'Residential'         => $freight_shipper_residential
			    )
			);

	    	$freight_request['CarrierCodes'] = 'FXFR';

	    	$freight_request['RequestedShipment']['FreightShipmentDetail'] = array(
				'FedExFreightAccountNumber'            => strtoupper( $freight_number ),
				'FedExFreightBillingContactAndAddress' => array(
					'Address'                             => array(
						'StreetLines'                        => array( strtoupper( $freight_billing_street ), strtoupper( $freight_billing_street_2 ) ),
						'City'                               => strtoupper( $freight_billing_city ),
						'StateOrProvinceCode'                => strtoupper( $freight_billing_state ),
						'PostalCode'                         => strtoupper( $freight_billing_postcode ),
						'CountryCode'                        => strtoupper( $freight_billing_country )
					)
				),
				'Role'                                 => 'SHIPPER',
				'PaymentType'                          => 'PREPAID',
			...
			foreach ( $freight_request['RequestedShipment']['RequestedPackageLineItems'] as $line_item ) {
				$total_weight += $line_item['Weight']['Value'];
			}

			$freight_request['RequestedShipment']['FreightShipmentDetail']['LineItems'] = array(
			...
			$freight_request['RequestedShipment']['ShippingChargesPayment'] = array(
				'PaymentType' => 'SENDER',
	            'Payor' => array(
					'ResponsibleParty' => array(
						'AccountNumber'           => strtoupper( $freight_number ),
			...
			// Add Package City
			$freight_request['RequestedShipment']['Recipient']['Address']['City'] = strtoupper( $package['destination']['city'] );
			//$freight_request['RequestedShipment']['Recipient']['Address']['City'] = strtoupper( 'Mooresville' );

			$freight_requests[] = $freight_request;
    	}
