...
$default_google_fonts = array( 'Open Sans', 'Merriweather' );
...
add_action( 'woo_head', 'woo_custom_styling', 20 ); // Add custom styling to HEAD
...
if ( ! function_exists( 'woo_custom_styling' ) ) {
	function woo_custom_styling() {
		...
		if ( $settings['content_color'] != '' ) {
			$output .= '.post, .type-page, .type-features, .search-results .features, .search-results .page, .search-results .type-product, .search-results .type-portfolio { background: ' . $settings['content_color'] . ' !important; }' . "\n";
		}

		if ( $settings['widget_color'] != '' ) {
			$output .= '.widget { background: ' . $settings['widget_color'] . ' !important; }' . "\n";
		}		

		if ( $settings['link_color'] != '' ) {
			$output .= 'a { color: ' . $settings['link_color'] . '; }' . "\n";
		}
...
add_action( 'woo_head','woo_custom_typography', 20 ); // Add custom typography to HEAD
	...
	function woo_custom_typography() {
		...
		if ( isset( $woo_options['woo_typography'] ) && $woo_options['woo_typography'] == 'true' ) {
			...
			if ( isset( $woo_options['woo_font_nav'] ) && $woo_options['woo_font_nav'] )
				$output .= '#navigation ul.nav > li a { '.woo_generate_font_css($woo_options['woo_font_nav'], '1.4').' }' . "\n";	

			if ( isset( $woo_options['woo_font_nav_home'] ) && $woo_options['woo_font_nav_home'] )
				$output .= '.home #navigation ul.nav > li a { '.woo_generate_font_css($woo_options['woo_font_nav_home'], '1.4').' }' . "\n";	

			if ( isset( $woo_options['woo_font_page_title'] ) && $woo_options['woo_font_page_title'] )
				$output .= '.type-page header h1 { '.woo_generate_font_css($woo_options[ 'woo_font_page_title' ], '1.387').' }' . "\n";

			if ( isset( $woo_options['woo_font_post_title'] ) && $woo_options['woo_font_post_title'] )
				$output .= '.post header h1, .post header h1 a:link, .post header h1 a:visited { '.woo_generate_font_css($woo_options[ 'woo_font_post_title' ], '1.387').' }' . "\n";	
			...
			if ( isset( $woo_options['woo_font_widget_titles'] ) && $woo_options['woo_font_widget_titles'] )
				$output .= '.widget h3 { '.woo_generate_font_css($woo_options[ 'woo_font_widget_titles' ], '1.387').' }'  . "\n";
...
if ( ! function_exists( 'woo_layout_body_class' ) ) {
	function woo_layout_body_class( $classes ) {
		...
		if ( function_exists( 'has_nav_menu' ) && has_nav_menu( 'top-menu' ) ) {
			$classes[] = 'with-top-menu';
		}

		if ( $woo_options['woo_features_area'] != 'true' ) {
			$classes[] = 'without-features';
		}

		if ( $woo_options['woo_featured'] != 'false' && ( is_home() || is_front_page() ) ) {
			$classes[] = 'has-slider';
		}
